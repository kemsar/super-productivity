...
| GitLab          | Yes    | Auto (closed) | Deprecated | Yes      | No       | No          | Project / group / all-assigned, scope | —           | Weight    | Yes       |
...
- **Scan mode:** Single project, an entire group (recurses through all subgroups so new projects and new assignments surface automatically on the next poll), or every issue assigned to you across the entire GitLab instance.
- **Status transitions:** Issues auto-marked as done when closed in GitLab.
- **Worklog:** Previously supported; now deprecated.
- **Comments:** Comment tracking with optional filter by username.
- **Filtering:** Project / group / all-assigned scan; scope (All, Assigned, Created — ignored in all-assigned mode, which always uses "Assigned to me"); custom filter parameters.
